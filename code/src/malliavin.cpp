...
double gamma_call_bs(Call x, BSmodel y, double G ){
    double S = exp((y.Rate(0.0,0.0)-0.5*pow(y.Sigma(0.0,0.0),2))*x.Horizon()+G*x.Horizon()*y.Sigma(0.0,0.0));
    return exp(-y.Rate(0.0,0.0)*x.Horizon())*x.payoff(S)*(pow(G*x.Horizon(),2)/(y.Sigma(0.0,0.0)*x.Horizon())-G*x.Horizon()-1/y.Sigma(0.0,0.0))/(pow(y.S(),2)*y.Sigma(0.0,0.0)*x.Horizon());
};

double vega_call_bs(Call x, BSmodel y, double G ){
    double S = exp((y.Rate(0.0,0.0)-0.5*pow(y.Sigma(0.0,0.0),2))*x.Horizon()+G*x.Horizon()*y.Sigma(0.0,0.0));
    return exp(-y.Rate(0.0,0.0)*x.Horizon())*x.payoff(S)*(pow(G*x.Horizon(),2)/(y.Sigma(0.0,0.0)*x.Horizon())-G*x.Horizon()-1/y.Sigma(0.0,0.0));
};
